<?php
/**
 * Inline lucide SVG icons — path data copied verbatim from lucide-react v0.543.0
 * (the version used by the medvault lovable design source).
 */

if (!function_exists('lucide')) {
    /**
     * @return array<string,string> shape => svg element name
     */
    function lucideIconSet(): array
    {
        static $icons = null;
        if ($icons !== null) {
            return $icons;
     ...
     */
    function lucide(string $name, string $class = 'icon-4'): string
    {
        $set = lucideIconSet();
        $inner = $set[$name] ?? null;
        if ($inner === null) {
        ...
        return '<svg class="' . $class . '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">' . $inner . '</svg>';
    }
}
