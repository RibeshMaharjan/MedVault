...
## Post-Phase-8 pixel audit (screenshot diff against live lovable mockup)

After Phase 8, the user asked for a stricter pass since visual gaps remained. Installed lovable's deps (`bun install`) and ran its dev server (`vite dev`), plus a headless Chromium (puppeteer-core driving the system `/usr/bin/chromium`) to screenshot every page of both apps side by side at 1440px and diff by eye. This surfaced real bugs the earlier route-status/lint checks couldn't catch:

- **3-column grids collapsed to full-width stacking**: `dashboard.php`, `analytics/orders.php`, and `profile.php` reused the 2-column `.grid-2col`/`.grid-2col--span2` classes for layouts that are actually `lg:grid-cols-3` in lovable (chart/form spans 2-of-3, not 2-of-2). A `span-2` item in a 2-col grid consumes the whole row, pushing the sibling to the next row — added `.grid-3col`/`.grid-3col--span2` and fixed all three views.
- **`.file-upload` used a `<label>` tag but never set `display:block`**, so it inherited the browser's default `display:inline` for labels, corrupting the dashed dropzone's box model. Fixed in `components.css`.
- **Sort-arrow affordance was missing everywhere**: `ui.js` had working `data-sort` click handling from Phase 1, but no view ever put `data-sort` on a `<th>`. Added a `sortableTh()` helper (`app/Helpers/functions.php`) and applied it to medicines/categories/orders/sales/admins/pharmacies tables, matching lovable's `ArrowUpDown` icon-button-in-header pattern exactly.
- **Toolbar filter forms had labels above every field**; lovable's toolbars are label-less (placeholder-only, single compact row). Restyled medicines/orders/sales toolbars to match (kept the extra filter fields the original PHP app already supported — expiry range, max stock — since those are real functionality, not just cosmetic).
- **Order-analytics donut was scoped to "last week"** (via the reused `/api/pharmacy/order-data?period=week` default) producing a monochrome/misleading chart, while lovable's mock shows the full status mix. Added a `period=all` case to `AjaxController::getOrderData()`.
- **Admin dashboard list cards were missing subtext** (email under pharmacy name, "Requested {date}" under verification-queue rows) and had slightly different copy/labels than lovable ("Dashboard" → "Platform overview", "Pending" → "Pending verification", unverified→pending status wording). Fixed controller query + view to match.
- Landing page: CTA checklist icons inherited the surrounding muted-gray text color instead of `--success` green (no explicit color override on the icon); hero `<h1>` had default (loose) line-height instead of tight, causing an oversized gap between the two heading lines. Both fixed.

**Unrelated environment discovery, not a redesign bug**: a pre-existing `.env` file (dated June 8, gitignored) points the live app at a remote Aiven-hosted MySQL instance; `App::boot()` only ever reads DB creds from that file, never from docker-compose's `environment:` block, so the app was talking to that remote DB the whole session, not the local `ui-redesign-db-1` container. This only mattered for building a repeatable local test login — flagged to the user, who approved temporarily renaming `.env` aside for local verification, then restoring it exactly afterward (done — verified restored, app container restarted to pick it back up).
